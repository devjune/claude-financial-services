...
이 스킬은 **발표·리뷰용**이다. 같은 시나리오는 **항상 같은 결과**를 내야 한다.

- **골든 파일이 진실이다.** `outputs/<scenario>.md`가 이미 있으면, 그게 **고정 정답**이다. 재생성할 땐 그 파일을 **그대로 재현**한다 — 섹션·표·수치·다이어그램 노드 텍스트까지 동일하게. 다르게 나오면 골든 파일이 이긴다.
- **새로 만들 땐 결정적으로.** 섹션 순서·표 컬럼·다이어그램 구조를 아래 가이드대로 **고정**한다. 자유 서술·창의적 표현·이모지 임의 추가 금지.
- **숫자는 규칙으로만.** 모든 수치는 시나리오 입력 + 명시된 계산 규칙에서 결정적으로 도출(반올림 자리수도 고정). 추정·변형 금지, 모르면 `[UNSOURCED]`.
- **표현을 고정.** 같은 의미는 같은 문구로. 문장을 매번 새로 쓰지 말 것.

> 즉, 발표 땐 커밋된 `outputs/`를 쓰고, 스킬은 "그 골든 결과를 어떻게 만드는가"의 명세다.
